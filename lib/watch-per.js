...
exports.run = function (projectRoot, watchOpts, cb) {
    ...
    var includes = _.flattenDeep(watchObj.includes).filter(function (i) { return i; }).forEach(function (v) {
        if (typeof v !== 'string') {
            throw includesErr;
        }
    });
    var excludes = _.flattenDeep(watchObj.excludes).filter(function (i) { return i; }).forEach(function (v) {
