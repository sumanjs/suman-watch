    ...
    return function transpile(f, transformData, isTranspile, cb) {
        ...
        su.makePathExecutable(transformPath, function (err) {
            ...
            k.once('exit', function (code) {
                if (code > 0) {
                    logging_1.default.warn('transpilation may have failed, the process has exited with code => ', code);
                }
                else {
                    logging_1.default.info('transpilation process appears successful, and has exited with code => ', code);
                }
                clearTimeout(to);
                if (!timedout) {
                    var err_1;
