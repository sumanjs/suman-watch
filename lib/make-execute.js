    ...
    return function (f, runData, $cb) {
        ...
        if (runData.config && runData.config.length > runLength) {
            try {
                ...
                logging_1.default.warn('no run plugin could be found to execute the test.');
            }
        }
        if (!runPath) {
            if (runData.run) {
                logging_1.default.warn('runPath has been found.');
                runPath = runData.run;
            }
            else {
                logging_1.default.warn('runPath has not been found.');
            }
        }
