...
exports.makeExecute = function (watchOptions, projectRoot) {
    return function (f, runData, $cb) {
        ...
        var handleStdioAndExit = function (k, runPath, f) {
            ...
            k.stdout.pipe(prepend_transform_1.pt(chalk.black.bold(' [watch-worker-exec] '))).pipe(process.stdout);
            k.stdout.on('data', function (d) {
                stdout += d;
            });
            var stderr = '';
            k.stderr.setEncoding('utf8');
            k.stderr.pipe(prepend_transform_1.pt(chalk.yellow(' [watch-worker-exec] '), { omitWhitespace: true })).pipe(process.stderr);
