...
check was introduced: **153 findings across 32 files
and 2,760 lines**. CI compares against that budget and fails only when a
...
|---|---:|---|---|---|
| `TS002` | 80 | warn | immutable values | mutable `let` binding |
| `TS005` | 22 | warn | explicit inputs | `any` escapes the type system |
| `TS009` | 17 | warn | explicit inputs | ambient impurity (clock / randomness / env) read inside pure code |
| `TS008` | 14 | warn | effects pushed outward | direct console effect in library code |
| `TS006` | 8 | warn | typed errors | `throw` as control flow |
| `TS004` | 6 | warn | pure transformations | in-place array mutation |
| `TS003` | 5 | error | explicit state transitions | module-level mutable binding |
...
*immutable values* · 80 occurrences at baseline
...
*explicit inputs* · 22 occurrences at baseline
...
*effects pushed outward* · 14 occurrences at baseline
...
*pure transformations* · 6 occurrences at baseline
...
*explicit state transitions* · 5 occurrences at baseline
